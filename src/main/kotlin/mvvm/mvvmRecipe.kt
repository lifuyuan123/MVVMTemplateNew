...
fun RecipeExecutor.mvvmRecipe(
    ...
    val packageRealName =
        if (packgeNames.size > 2) "${packgeNames[0]}.${packgeNames[1]}.${packgeNames[2]}" else packageName
    ...
    if (needActivity && generateActivityLayout) {
        save(
            templateXml("${activityPackageName}.${pageName}Activity", "activity"),
            resOut.resolve("layout/${activityLayoutName}.xml")
        )
    }

    if (needFragment && generateFragmentLayout) {
        save(
            templateXml("${fragmentPackageName}.${pageName}Fragment"),
        ...
        if (ktOrJavaExt == "java")
            save(
                mvvmFragmentJava(
                    ...
                    packageRealName,
                    fragmentLayoutName
                ), srcOut.resolve("${pageName.toLowerCase()}/${pageName}Fragment.${ktOrJavaExt}")
            )
        else
            save(
                mvvmFragmentKt(
    ...
    if (needRepository) {
        if (ktOrJavaExt == "java")
            save(
                mvvmRepositoryJava(repositoryPackageName, "", "", needActivity, needFragment),
                srcOut.resolve("mvp/presenter/${pageName}Presenter.${ktOrJavaExt}")
            )
        else
            save(
                mvvmRepositoryKt(repositoryPackageName, pageName, packageRealName),
                srcOut.resolve("${pageName.toLowerCase()}/${pageName}Repository.${ktOrJavaExt}")
            )
    }
    ...
    if (needViewModel) {
        if (ktOrJavaExt == "java")
            save(
                mvvmViewModelJava(viewModelPackageName, "", "", needActivity, needFragment),
                srcOut.resolve("${pageName}/${pageName}ViewModel.${ktOrJavaExt}")
            )
        else
            save(
                mvvmViewModelKt(viewModelPackageName, pageName, packageRealName),
                srcOut.resolve("${pageName.toLowerCase()}/${pageName}ViewModel.${ktOrJavaExt}")
            )
    }
